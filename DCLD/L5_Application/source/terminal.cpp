...
bool terminalTask::taskEntry()
{
    ...
    cp.addHandler(listHandler, "list", "list sym/range/var/fun, Print list of symbols, range, variables, functions");
    cp.addHandler(readHandler, "read", "read var, Read the value of a variable");
    cp.addHandler(assignHandler, "assign", "assign var val, Assign a value to a variable at run-time");
    cp.addHandler(execHandler, "exec", "exec fun_name, Execute a function in run-time");
